...
# =============================================================================
# Legacy resources (to be destroyed after migration)
# =============================================================================

# Old storybook-only IP (34.160.242.136) — keep until DNS migrates
resource "google_compute_global_address" "storybook_legacy" {
  name = "storybook"
}

# Old storybook-only SSL cert
resource "google_compute_managed_ssl_certificate" "storybook_legacy" {
  name = "storybook"

  managed {
    domains = ["storybook.squadquest.app"]
  }
}

# Old storybook-only URL map
resource "google_compute_url_map" "storybook_legacy" {
  name            = "storybook"
  default_service = google_compute_backend_bucket.storybook.id
}

# Old HTTP→HTTPS redirect URL map
resource "google_compute_url_map" "storybook_legacy_redirect" {
  name = "squadquest-redirect"
...
# Old HTTPS proxy
resource "google_compute_target_https_proxy" "storybook_legacy" {
  name             = "storybook-target-proxy-2"
  url_map          = google_compute_url_map.storybook_legacy.id
  ssl_certificates = [google_compute_managed_ssl_certificate.storybook_legacy.id]
}

# Old HTTP proxy
resource "google_compute_target_http_proxy" "storybook_legacy" {
  name    = "squadquest-target-proxy"
  url_map = google_compute_url_map.storybook_legacy_redirect.id
}

# Old forwarding rules using legacy IP
resource "google_compute_global_forwarding_rule" "storybook_legacy_https" {
  name                  = "squadquest"
  target                = google_compute_target_https_proxy.storybook_legacy.id
  ip_address            = google_compute_global_address.storybook_legacy.id
  port_range            = "443"
  load_balancing_scheme = "EXTERNAL_MANAGED"
}

resource "google_compute_global_forwarding_rule" "storybook_legacy_http" {
  name                  = "squadquest-forwarding-rule"
  target                = google_compute_target_http_proxy.storybook_legacy.id
  ip_address            = google_compute_global_address.storybook_legacy.id
  port_range            = "80"
  load_balancing_scheme = "EXTERNAL_MANAGED"
}
